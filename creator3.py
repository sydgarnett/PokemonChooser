...
class Creator3():
    ...
    def birthTotal(self):
        l=len(self.name)-1
        if self.month<10:
            return self.month+((self.day//10)+(self.day%10))+self.year+(l)
        else:
            return (self.month/10)+((self.day//10)+(self.day%10))+self.year+(l)
    def decimal(self):
        deci=0.00
        ...
        d=(float)(b*.1)
        deci+=d;
        lastLettF=self.getFirstName()[len(self.getFirstName())-1:]
        h=self.countSet(lastLettF)
        g=float(h**.3)
        deci+=g;
        return deci;
    def FinalNum(self):
